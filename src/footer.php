<footer class="footer bg-dark text-white py-4 mt-auto">
    <div class="container">
        <div class="row align-items-center gy-3">
            <div class="col-md-4 text-md-start text-center">
                <h5 class="mb-1">Farmacia Sistema</h5>
                <p class="small text-muted mb-0">Gestión profesional de farmacias</p>
            </div>
            
            <div class="col-md-4 text-center">
                <div class="d-flex justify-content-center gap-3 mb-2">
                    <a href="#" class="text-decoration-none text-white">
                        <i class="bi bi-facebook fs-5"></i>
                    </a>
                    <a href="#" class="text-decoration-none text-white">
                        <i class="bi bi-whatsapp fs-5"></i>
                    </a>
                    <a href="#" class="text-decoration-none text-white">
                        <i class="bi bi-instagram fs-5"></i>
                    </a>
                    <a href="#" class="text-decoration-none text-white">
                        <i class="bi bi-envelope fs-5"></i>
                    </a>
                </div>
                <p class="small mb-0">&copy; <?= date('Y') ?> InCode - Todos los derechos reservados</p>
            </div>
            
            <div class="col-md-4 text-md-end text-center">
                <a href="" class="btn btn-outline-light btn-sm mb-2">
                    <i class="bi bi-chat-text me-1"></i>Contacto
                </a>
                <p class="small mb-0">
                    <a href="" class="text-white text-decoration-none small">Política de Privacidad</a> |
                    <a href="" class="text-white text-decoration-none small">Términos de Uso</a>
                </p>
            </div>
        </div>
    </div>
</footer>